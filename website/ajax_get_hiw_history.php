<?php
// Fetch how-it-works page edit history (super admin only)
session_start();
...
$sql="SELECT audit_id, block_key, action_type, new_html AS html, new_text_color AS text_color, new_bg_color AS bg_color, admin_username, created_at FROM how_it_works_content_audit WHERE municipality_id=1".(count($where)?' AND '.implode(' AND ',$where):'')." ORDER BY created_at DESC LIMIT $limit";
