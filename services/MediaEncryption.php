<?php
/**
 * MediaEncryption service (Version 2 upgrade)
...
class MediaEncryption
{
    ...
    private function loadKeys(): void
    {
        ...
        if ($multi) {
            ...
            if (!empty($this->keys)) {
                ...
                $override = getenv('MEDIA_ENCRYPTION_ACTIVE_KEY');
    ...
    /**
     * Encrypt data using Version 2 format (always) if enabled; throws if disabled.
     * @throws \RuntimeException when encryption not configured.
     */
    ...
    /**
     * Decrypt data (auto-detect version). Returns array(mime,data,encrypted).
     */
    ...
    {
        $len = strlen($blob);
        if ($offset >= $len) return ['mime'=>'application/octet-stream','data'=>'','encrypted'=>true];
        $mimeLen = ord($blob[$offset++]);
        if ($offset + $mimeLen > $len) return ['mime'=>'application/octet-stream','data'=>'','encrypted'=>true];
        $mime = substr($blob, $offset, $mimeLen); $offset += $mimeLen;
        if ($offset >= $len) return ['mime'=>$mime,'data'=>'','encrypted'=>true];
        $ivLen = ord($blob[$offset++]);
        if ($offset + $ivLen > $len) return ['mime'=>$mime,'data'=>'','encrypted'=>true];
        $iv = substr($blob, $offset, $ivLen); $offset += $ivLen;
        if ($offset >= $len) return ['mime'=>$mime,'data'=>'','encrypted'=>true];
        $tagLen = ord($blob[$offset++]);
        if ($offset + $tagLen > $len) return ['mime'=>$mime,'data'=>'','encrypted'=>true];
    ...
    {
        $len = strlen($blob);
        if ($offset + 4 > $len) return ['mime'=>'application/octet-stream','data'=>'','encrypted'=>true];
        ...
        $tryKeys = [];
        if (isset($this->keys[$keyId])) { $tryKeys[$keyId] = $this->keys[$keyId]; }
        foreach ($this->keys as $id => $k) { if (!isset($tryKeys[$id])) $tryKeys[$id] = $k; }
